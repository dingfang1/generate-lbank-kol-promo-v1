...
- In `filled_template.xlsx`, source activity sheets keep their original `配图` cells, including WPS `DISPIMG(...)` cell-image formulas, so users can visually check which source image belongs to each language row.
